...
class Residual(nn.Module):
    ...
    def forward(self, x):
        return x + self.bn2(self.conv2(self.pad(F.relu(self.bn1(self.conv1(self.pad(x)))))))
...
class Net(nn.Module):
    ...
    def encoder(self, x):
        ...
        shape = F.relu(self.bn_shape_1(self.conv_shape_2(shape)))
    ...
    def decoder(self, text, shape):  # tensor: N(batchsize)*C(channel)*H*W
        ...
        x = self.pad_3(x)
        x = F.relu(self.bn_output_2(self.conv_output_2(x)))
        return x
